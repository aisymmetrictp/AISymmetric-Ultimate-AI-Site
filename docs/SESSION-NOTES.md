...
### Lessons learned
- **CSS `animation-fill-mode: both` is a footgun for entrance animations.** Caused the /success hero to render blank in some browsers — the `from` state of `opacity: 0` stayed sticky. Fixed by removing the entrance animation entirely from the hero.
- **IntersectionObserver-driven scroll-reveal is unreliable for prospect-facing pages.** After the hero fix, below-the-fold content (57 elements) was still invisible because the observer wasn't firing correctly — verified in a fresh headless Chrome, `revealed: 0`, `stillHidden: 57`. Removed the scroll-reveal mechanism entirely. Content now visible by default with no JS dependency for visibility.
- **Rule going forward: no JS-dependent visibility for ANY content, above or below the fold.** If the JS fails, the CSS animation stalls, or the browser has an edge-case render bug, content MUST still show. Progressive enhancement only — never progressive-require. Entrance animations are a nice-to-have that carry outsized reliability risk; skip them on prospect-facing pages.
- **Debug broken renders by spinning up a clean headless browser on the live URL.** Takes 2 minutes with Claude in Chrome; proved the server-side was fine and narrowed the bug to the specific JS pattern. Much faster than guessing.
- **Browser cache was a red herring here.** User's `no-store` cache headers on `/success/*` are still a good idea (added 2026-04-21) but the ACTUAL bug was the JS reveal logic, not a caching issue.
